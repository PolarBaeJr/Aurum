    ...
    public static void loadFromFile() {
        try {
            ...
            if (itemFile.exists()) {
                ...
                Map<String, String> encodedMap = (Map<String, String>)gson.fromJson((Reader)reader, type);
                if (encodedMap == null) {
                    itemMap = new HashMap<String, ItemStack>();
                } else {
                    itemMap.clear();
                    for (Map.Entry<String, String> entry : encodedMap.entrySet()) {
                        itemMap.put((String)entry.getKey(), ItemDataManager.decodeItem((String)entry.getValue()));
                    }
                }
            }
            if (groupFile.exists()) {
                ...
                Map groups = (Map)gson.fromJson((Reader)reader2, type2);
                groupMap = groups == null ? new HashMap() : groups;
            }
        }
        catch (IOException e) {
            e.printStackTrace();
