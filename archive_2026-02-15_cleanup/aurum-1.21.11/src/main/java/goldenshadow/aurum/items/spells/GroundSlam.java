...
extends PlayerSpell {
    ...
    public GroundSlam(Player player) {
        super(player);
        this.damage = Math.max(0.0, Objects.requireNonNull(player.getAttribute(Attribute.ATTACK_DAMAGE)).getValue() * 0.5 * this.getDamageMultiplier());
    }
    ...
    public void tick() {
        ...
        if (this.getTick() > 20) {
            ...
            if (!this.getPlayer().getWorld().getBlockAt(this.getPlayer().getLocation().add(0.0, -0.1, 0.0)).isPassable()) {
                this.getPlayer().getWorld().spawnParticle(Particle.EXPLOSION, this.getPlayer().getLocation(), 0);
