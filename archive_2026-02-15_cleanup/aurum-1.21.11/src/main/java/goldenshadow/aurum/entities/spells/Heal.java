...
extends Spell {
    ...
    public void tick() {
        super.tick();
        if (this.getTick() == 21) {
            double maxHealth = Objects.requireNonNull(this.getEntity().getAttribute(Attribute.MAX_HEALTH)).getValue();
            this.getEntity().setHealth(Math.min(this.getEntity().getHealth() + maxHealth * 0.1, maxHealth));
            this.getEntity().getWorld().spawnParticle(Particle.HAPPY_VILLAGER, this.getEntity().getEyeLocation(), 20, 0.3, 0.5, 0.3, 0.0);
