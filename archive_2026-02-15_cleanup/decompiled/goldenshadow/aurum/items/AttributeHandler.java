...
public class AttributeHandler {
    ...
    public static void meleeHit(EntityDamageByEntityEvent event) {
        ...
        if (i.getItemMeta() != null && i.getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)Aurum.getPlugin(), "minLevel"), PersistentDataType.INTEGER)) {
            event.setDamage(Objects.requireNonNull(player.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE)).getValue() * (AttributeHandler.isCritical(player) ? 1.5 : 1.0));
        }
        ...
        if ((entity = event.getEntity()) instanceof LivingEntity) {
            ...
            if (itemHelper.isAttributeEquipped(player, AttributeID.EXPLODING) && (double)ThreadLocalRandom.current().nextInt(0, 101) <= itemHelper.getAttributeRoll(player, AttributeID.EXPLODING, false)) {
                ...
                entity2.getWorld().playSound(location, Sound.ENTITY_GENERIC_EXPLODE, 1.0f, 1.0f);
                entity2.getWorld().spawnParticle(Particle.EXPLOSION_LARGE, location, 3);
                object = event.getEntity().getNearbyEntities(1.0, 1.0, 1.0).iterator();
                while (object.hasNext()) {
                    nearbyEntity = (Entity)object.next();
                    if (!(nearbyEntity instanceof LivingEntity) || !nearbyEntity.getScoreboardTags().contains("aurum_mob")) continue;
                    entity2.damage(event.getDamage() * 0.6, (Entity)player);
                }
            }
            ...
            if (itemHelper.isAttributeEquipped(player, AttributeID.FREEZING) && (double)ThreadLocalRandom.current().nextInt(0, 101) <= itemHelper.getAttributeRoll(player, AttributeID.FREEZING, false)) {
                ...
                entity2.getWorld().spawnParticle(Particle.SNOWFLAKE, location, 20, 0.2, 2.0, 0.2);
            }
            if (itemHelper.isAttributeEquipped(player, AttributeID.LIFE_STEAL) && RuneAbilityHandler.isBloodrushNotActive(player) && (double)ThreadLocalRandom.current().nextInt(0, 101) <= itemHelper.getAttributeRoll(player, AttributeID.LIFE_STEAL, false)) {
                ...
                player.setHealth(Math.min(player.getHealth() + stolen, Objects.requireNonNull(player.getAttribute(Attribute.GENERIC_MAX_HEALTH)).getValue()));
            }
            if (itemHelper.isAttributeEquipped(player, AttributeID.CRITICAL_STRIKE) && (double)ThreadLocalRandom.current().nextInt(0, 101) <= itemHelper.getAttributeRoll(player, AttributeID.CRITICAL_STRIKE, false)) {
                ...
                entity2.getWorld().spawnParticle(Particle.SPELL_WITCH, event.getEntity().getLocation(), 10, 0.0, 2.0, 0.0);
            }
            ...
            if ((object = event.getEntity()) instanceof Player) {
                Player attackedPlayer = (Player)object;
                if (itemHelper.isAttributeEquipped(attackedPlayer, AttributeID.REBOUND) && (double)ThreadLocalRandom.current().nextInt(0, 101) <= itemHelper.getAttributeRoll(attackedPlayer, AttributeID.REBOUND, false) && (nearbyEntity = event.getDamager()) instanceof LivingEntity) {
                    LivingEntity attacker = (LivingEntity)nearbyEntity;
                ...
                if (itemHelper.isAttributeEquipped(attackedPlayer, AttributeID.REFLECTION) && (double)ThreadLocalRandom.current().nextInt(0, 101) <= itemHelper.getAttributeRoll(player, AttributeID.REFLECTION, false) && (nearbyEntity = event.getDamager()) instanceof LivingEntity) {
                    ...
                    if (itemHelper.isHighEnoughLevel(attackedPlayer, player.getInventory().getItemInMainHand())) {
                        AttributeInstance a = attackedPlayer.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE);
                        assert (a != null);
    ...
    public static void playerDamageTaken(EntityDamageEvent event) {
        ...
        if (itemHelper.isHighEnoughLevel(player, player.getInventory().getItemInMainHand())) {
            ...
            if (itemHelper.isAttributeEquipped(player, AttributeID.RESISTANCE)) {
                ...
                if (roll >= 0.0) {
                    for (roll = itemHelper.getAttributeRoll(player, AttributeID.RESISTANCE, false); roll > 0.0; roll -= 10.0) {
                        if (roll > 100.0) {
                            multiplier -= 0.01;
                            continue;
                        }
                    ...
                    while (roll < 0.0) {
                        multiplier = roll < -100.0 ? (multiplier += 0.01) : (multiplier += 0.05);
                        roll += 10.0;
                    }
                }
    ...
    public static void attributeLoop() {
        for (Player player : Bukkit.getServer().getOnlinePlayers()) {
            ...
            PotionEffect jump_height = new PotionEffect(PotionEffectType.JUMP, 40, (int)(itemHelper.getAttributeRoll(player, AttributeID.JUMP_HEIGHT, false) - 1.0), false, false);
    ...
    private static void applyCooldown(Player player) {
        ...
        if (i.getItemMeta() != null && i.getItemMeta().getPersistentDataContainer().has(new NamespacedKey((Plugin)Aurum.getPlugin(), "minLevel"), PersistentDataType.INTEGER)) {
            ...
            if (attackTier == 1) {
                ...
                player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, 40, 3, false, false));
            }
            if (attackTier == 0) {
                player.setCooldown(i.getType(), 15);
                player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, 40, 5, false, false));
            }
            if (attackTier == -1) {
                player.setCooldown(i.getType(), 25);
                player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, 40, 7, false, false));
            }
            if (attackTier <= -2) {
                player.setCooldown(i.getType(), 40);
                player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, 40, 9, false, false));
            }
        }
    }
}
