...
extends PlayerSpell {
    ...
    public void tick() {
        ...
        if (this.counter > 20) {
            ...
            for (Player p : Bukkit.getOnlinePlayers()) {
                double newHealth;
                if (this.getPlayer().getWorld() != p.getWorld() || !(this.getPlayer().getLocation().distance(p.getLocation()) < 6.0) || !RuneAbilityHandler.isBloodrushNotActive(p) || (newHealth = Math.min(p.getHealth() + 2.0, Objects.requireNonNull(p.getAttribute(Attribute.GENERIC_MAX_HEALTH)).getValue())) == p.getHealth()) continue;
    ...
    private void drawCircle(Location location) {
        for (double angle = 0.0; angle < Math.PI * 2; angle += 0.08726646259971647) {
            ...
            location.getWorld().spawnParticle(Particle.REDSTONE, location, 0, (Object)new Particle.DustOptions(Color.fromRGB((int)215, (int)79, (int)219), 2.0f));
