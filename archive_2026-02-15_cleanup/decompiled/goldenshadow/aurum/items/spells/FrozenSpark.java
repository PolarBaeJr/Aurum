...
extends PlayerSpell {
    ...
    public FrozenSpark(Player player) {
        ...
        this.damage = Math.max(0.0, Objects.requireNonNull(player.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE)).getValue() * 0.7 * this.getDamageMultiplier());
    }
    ...
    public void tick() {
        ...
        if (this.getTick() == 1) {
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                loc.getWorld().spawnParticle(Particle.BLOCK_CRACK, loc, 20, 0.2, 0.2, 0.2, (Object)this.block);
