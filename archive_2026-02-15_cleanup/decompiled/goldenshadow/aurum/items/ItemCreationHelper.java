...
public class ItemCreationHelper {
    ...
    public ItemMeta addAttribute(ItemMeta meta, int roll, AttributeID attribute, List<String> lore, ItemType itemType) {
        ...
        if (attribute == AttributeID.HEALTH) {
            meta.addAttributeModifier(Attribute.GENERIC_MAX_HEALTH, new AttributeModifier(UUID.randomUUID(), "generic_max_health", (double)roll + this.getAttributeSum(Attribute.GENERIC_MAX_HEALTH, meta), AttributeModifier.Operation.ADD_NUMBER, this.parseItemType(itemType)));
        } else if (attribute == AttributeID.WALK_SPEED) {
            double amount = (double)roll / 100.0;
            meta.addAttributeModifier(Attribute.GENERIC_MOVEMENT_SPEED, new AttributeModifier(UUID.randomUUID(), "generic_movement_speed", 0.1 * amount, AttributeModifier.Operation.ADD_NUMBER, this.parseItemType(itemType)));
        }
    ...
    public ItemMeta insertAttribute(ItemMeta meta, int roll, AttributeID attribute, List<String> lore, Material material) {
        ...
        if (attribute == AttributeID.HEALTH) {
            meta.addAttributeModifier(Attribute.GENERIC_MAX_HEALTH, new AttributeModifier(UUID.randomUUID(), "generic_max_health", (double)roll + this.getAttributeSum(Attribute.GENERIC_MAX_HEALTH, meta), AttributeModifier.Operation.ADD_NUMBER, this.parseMaterial(material)));
        } else if (attribute == AttributeID.WALK_SPEED) {
            double amount = (double)roll / 100.0;
            meta.addAttributeModifier(Attribute.GENERIC_MOVEMENT_SPEED, new AttributeModifier(UUID.randomUUID(), "generic_movement_speed", 0.1 * amount, AttributeModifier.Operation.ADD_NUMBER, this.parseMaterial(material)));
        }
        meta = this.addAttributeData(meta, attribute, roll);
        meta.setLore(lore);
        return meta;
    ...
    public EquipmentSlot parseItemType(ItemType itemType) {
        return switch (itemType) {
            case ItemType.HELMET -> EquipmentSlot.HEAD;
            case ItemType.CHESTPLATE -> EquipmentSlot.CHEST;
            case ItemType.LEGGINGS -> EquipmentSlot.LEGS;
            case ItemType.BOOTS -> EquipmentSlot.FEET;
            default -> EquipmentSlot.HAND;
        };
    }

    public EquipmentSlot parseMaterial(Material material) {
    ...
    public ItemMeta addAttributeData(@NotNull ItemMeta meta, AttributeID attribute, int roll) {
        meta.getPersistentDataContainer().set(AttributeKeys.getKey(attribute), PersistentDataType.INTEGER, (Object)roll);
        return meta;
    }
    ...
    public ItemMeta addRuneData(@NotNull ItemMeta meta, Rune rune) {
        ...
        if (rune == Rune.EMPTY && (i = (Integer)meta.getPersistentDataContainer().get(RuneKeys.getKey(Rune.EMPTY), PersistentDataType.INTEGER)) != null) {
            meta.getPersistentDataContainer().set(RuneKeys.getKey(Rune.EMPTY), PersistentDataType.INTEGER, (Object)(i + 1));
            return meta;
        }
        meta.getPersistentDataContainer().set(RuneKeys.getKey(rune), PersistentDataType.INTEGER, (Object)1);
        return meta;
    }
