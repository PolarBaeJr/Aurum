...
public class ExperienceManager {
    ...
    public static void resetLevelAndXP(Player player) {
        PersistentDataContainer data = player.getPersistentDataContainer();
        data.set(combatXP, PersistentDataType.INTEGER, (Object)0);
        data.set(combatLevel, PersistentDataType.INTEGER, (Object)1);
    ...
    public static void setXP(Player player, int value) {
        if (player.getPersistentDataContainer().has(combatXP, PersistentDataType.INTEGER)) {
            player.getPersistentDataContainer().set(combatXP, PersistentDataType.INTEGER, (Object)value);
            ExperienceManager.visualise(player);
        }
    }
    ...
    public static void setLevel(Player player, int value) {
        if (player.getPersistentDataContainer().has(combatLevel, PersistentDataType.INTEGER)) {
            player.getPersistentDataContainer().set(combatLevel, PersistentDataType.INTEGER, (Object)value);
        ...
        if (player.getPersistentDataContainer().has(combatXP, PersistentDataType.INTEGER)) {
            ...
            if (xp != null && level != null) {
                ...
                if ((xp = Integer.valueOf(xp + value)) >= xpForNextLevel) {
                    ...
                    player.getPersistentDataContainer().set(combatXP, PersistentDataType.INTEGER, (Object)0);
                    player.getPersistentDataContainer().set(combatLevel, PersistentDataType.INTEGER, (Object)level);
                    ExperienceManager.levelUp(player, level);
                    ExperienceManager.addXP(player, xp, false);
                } else {
                    player.getPersistentDataContainer().set(combatXP, PersistentDataType.INTEGER, (Object)xp);
                }
                ExperienceManager.visualise(player);
            }
        }
