...
extends Spell {
    ...
    private final double damage = Objects.requireNonNull(this.getEntity().getAttribute(Attribute.GENERIC_ATTACK_DAMAGE)).getValue() * 2.5 * this.getSpellDamageMultiplier();
    ...
    public void tick() {
        ...
        if (this.getTick() > 21 && this.getTick() < 30) {
            this.getEntity().getWorld().spawnParticle(Particle.SPELL_WITCH, this.getEntity().getLocation(), 10, 0.4, 0.0, 0.4, 0.0);
        ...
        if (this.getTick() >= 30 && this.getTick() <= 35) {
            this.getEntity().getWorld().spawnParticle(Particle.REDSTONE, this.getEntity().getEyeLocation(), 30, 0.3, 0.5, 0.3, (Object)new Particle.DustOptions(Color.fromRGB((int)235, (int)52, (int)186), 1.0f));
            this.damageNearby();
        }
