...
public class DataManager {
    ...
    public static void loadFromFile() throws FileNotFoundException {
        ...
        if (locationsFile.exists() && (locArray = gson.fromJson((Reader)(locationsReader = new FileReader(locationsFile)), SpawnLocation[].class)) != null) {
            locationList = new ArrayList<Object>(Arrays.asList(locArray));
        }
        if (respawnFile.exists() && (locArray = gson.fromJson((Reader)(respawnReader = new FileReader(respawnFile)), RespawnLocation[].class)) != null) {
            respawnLocationList = new ArrayList<Object>(Arrays.asList(locArray));
        }
            ...
            for (int i = 0; i < 4; ++i) {
                for (SpawnLocation location : splitList.get(i)) {
                    for (Entity entity : location.getWorld().getNearbyEntities(location.getLocation(), 30.0, 30.0, 30.0, x -> x.getScoreboardTags().contains("aurum_debug_nodes"))) {
                        ...
                        player.spawnParticle(Particle.REDSTONE, location.getLocation(), 10, (Object)dustOptions);
                    }
                }
            }
        }
